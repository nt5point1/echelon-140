...
async function waitForElement(query, parent = null, timeout = 10000)
{
	// If parent is not provided, get the current browser window's document
	if (!parent) {
		let win = Services.wm.getMostRecentWindow("navigator:browser");
		if (!win) {
			// If no window exists yet, wait for one
			return new Promise((resolve) => {
				Services.obs.addObserver(function observer(subject) {
					Services.obs.removeObserver(observer, "browser-delayed-startup-finished");
					waitForElement(query, null, timeout).then(resolve);
	...
	let win = parent.ownerGlobal || parent.defaultView;
...
export { PrefUtils, BrandUtils, waitForElement, renderElement, VersionUtils, ThemeUtils };
